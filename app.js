        ...
        const YENILIKLER = [
            ...
            { faz: 'Günlük Kullanım', ad: 'Toleranslı (fuzzy) arama, sepetle toplu besin ekleme, akıllı öğün varsayılanı, muaf gün işaretleme, hızlı sayı ayarlayıcıları' },
...
const NUTRIO_SEED_V3_IZLENIM = {
        ...
        function profilMigrasyon() {
            ...
            profiller.forEach(p => {
                ...
                // FAZ 17 — eski varsayılan sıralama (Faz11'in orijinal düzeni: Skor/Aktivite/Su/Öğünler)
                // yeni kanonik sıraya (Skor/Öğünler/Su/Aktivite) taşınır. SADECE dizi TAM OLARAK bu
                // bilinen, hiç değiştirilmemiş eski desenle eşleşiyorsa değiştirilir — kullanıcının
                // Ayarlar'dan kendi elleriyle düzenlediği HERHANGİ bir sıralamaya asla dokunulmaz.
                if (JSON.stringify(p.kartSirasi) === JSON.stringify(['skor', 'aktivite', 'su', 'ogunler'])) {
                    p.kartSirasi = ['skor', 'ogunler', 'su', 'aktivite'];
                    degisti = true;
                }
                if (!p.kiloGecmisi) {
        ...
        const ICECEK_ADLARI = { cay: 'Çay', kahve_filtre: 'Kahve (filtre)', kahve_espresso: 'Kahve (espresso)', ice_tea: 'Ice Tea', enerji: 'Enerji İçeceği', gazli: 'Gazlı İçecek', diger: 'Diğer' };
        const ICECEK_KAFEIN_100ML = { cay: 20, kahve_filtre: 40, kahve_espresso: 212, ice_tea: 5, enerji: 32, gazli: 10, diger: 0 };
        ...
        function gecmisListele() {
            ...
            if (aktif.gecmis.length === 0) {
                liste.innerHTML = bosDurumHtml('🗂', 'Arşiv boş', 'Geçmiş günlere ait kayıtlar burada birikir. Bugün bir şeyler kaydet ve ilk arşiv gününü oluştur.') +
                    '<button class="btn-ikincil" onclick="gunDetayAc(tarihAyarla(bugununTarihi, -1))" style="margin-top:12px;">+ Geçmiş Bir Güne Kayıt Ekle</button>';
                return;
            }
        ...
        function gunDetayGuncelle() {
            ...
            // FAZ 10 — muaf gün toggle'ı: başlığın yanında rozet/kaldırma seçeneği
            const ozelGunAlan = document.getElementById('gun-detay-ozel-gun-alan');
            if (ozelGunAlan) {
                ozelGunAlan.innerHTML = gunOzelMi(tarih)
                    ? '<span style="font-size:13px; font-weight:700; color:var(--vurgu-renk); white-space:nowrap;">' + OZEL_GUN_IKON + ' Muaf Gün</span>' +
                    ...
                    : '<button class="btn-ikincil btn-kucuk" style="flex:0 0 auto;" onclick="gunOzelGunToggle(gunDetayAktifTarih || bugununTarihi)">' + OZEL_GUN_IKON + ' Muaf Tut</button>';
            }
        ...
        // FAZ 17 — "Özel Gün" → "Muaf Gün" olarak yeniden adlandırıldı, modern SVG ikon
        // (takvim + çapraz çizgi = "bu gün istatistiklere sayılmıyor" görseli).
        const OZEL_GUN_IKON = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line><line x1="8" y1="14" x2="16" y2="18"></line></svg>';

        // FAZ 17 — Ana ekrandan da bugünü tek tıkla Muaf Gün işaretleyebilme.
        function anaOzelGunAlaniGuncelle() {
            const alan = document.getElementById('ana-ozel-gun-alani');
            if (!alan) return;
            const ozel = gunOzelMi(bugununTarihi);
            alan.innerHTML = ozel
                ? '<span class="gun-tipi-rozet muaf" style="cursor:pointer;">' + OZEL_GUN_IKON + ' Bugün Muaf — istatistiklere sayılmıyor, dokun kaldır</span>'
                : '<span class="hedef-yazi" style="cursor:pointer; text-decoration:underline;">+ Bugünü Muaf Tut</span>';
            alan.onclick = () => gunOzelGunToggle(bugununTarihi);
            // FAZ 17 — kullanıcı uyarıyı okumadan da anlasın diye Skor Paneli'ne görsel bir işaret:
            // muaf günlerde kesikli, soluk bir kenarlık.
            const skorKart = document.getElementById('kart-skor');
            if (skorKart) skorKart.classList.toggle('muaf-gun-kart', ozel);
        }
        ...
        function gunOzelGunToggle(tarih) {
            ...
            if (aktif.ozelGunler.includes(tarih)) {
                ...
                bildirGoster('Muaf işareti kaldırıldı — gün yeniden ortalamalara dahil');
            } else {
                aktif.ozelGunler.push(tarih);
                bildirGoster('Bu gün muaf tutuldu: silinmiyor, sadece Trend/Analiz/Haftalık Karşılaştırma ortalamalarına DAHİL EDİLMİYOR.');
            }
            localStorage.setItem('df_profiller', JSON.stringify(profiller));
            const gdEkrani = document.getElementById('gun-detay-ekrani');
